...
# collect summary json paths for final table
SUMMARY_FILES=()
...
for entry in "${DATASET_REGISTRY[@]}"; do
  ...
  # check that train csv and checkpoint exist; skip gracefully if not
  ...
  # disable structure metrics when no WT PDB is available
  ...
  for METHOD in ess tess; do
    ...
    # pick the right delta flag per method
    if [[ "$METHOD" == "ess" ]]; then
...
if [[ ${#SUMMARY_FILES[@]} -gt 0 ]]; then
  ...
  for entry in "${SUMMARY_FILES[@]}"; do
    # entry format: DATASET__METHOD__path/to/results.summary.json
    DS="${entry%%__*}"
    rest="${entry#*__}"
    MTH="${rest%%__*}"
    JSON_PATH="${rest#*__}"
