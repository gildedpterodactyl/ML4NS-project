    ...
    with open(save_path, "w") as f:
        if prot_pos.ndim == 4:
            for t, pos37 in enumerate(prot_pos):
                atom37_mask = np.sum(np.abs(pos37), axis=-1) > 1e-4
            ...
            atom37_mask = np.sum(np.abs(prot_pos), axis=-1) > 1e-4
            prot = create_full_prot(
...
def extract_ca(x: torch.Tensor) -> torch.Tensor:
    """Extract CA coordinates from the input tensor.

    Args:
      x: Input tensor of shape [b, 4n, 3].

    Returns:
      CA coordinates of shape [b, n, 3].
    """
    assert x.shape[-1] == 3, f"Expected coords dimension to be 3, got {x.shape[-1]} {x.shape}"
    assert x.shape[-2] % 4 == 0, f"Expected backbone dimension to be divisible by 4, got {x.shape[-2]} {x.shape}"
    return rearrange(x, "b (n c) d -> b n c d", c=4)[..., 1, :]