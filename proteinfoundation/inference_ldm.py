...
class GenDataset(Dataset):
    ...
    def __init__(self, nres=[110], dt=0.005, dt_latent=0.005, nsamples=10, len_cath_codes=None):
...
if __name__ == "__main__":
    ...
    if not cfg.lora.use:
        ...
        model = ProteinLDM.load_from_checkpoint(ckpt_file, strict=False)
    ...
    if (
        ...
        model_ag = ProteinLDM.load_from_checkpoint(ckpt_ag_file)
        nn_ag = model_ag.nn
    ...
    if cfg.fold_cond:
        ...
        len_cath_codes = None
    dataset = GenDataset(
        nres=lens_sample, nsamples=nsamples, dt=cfg.dt, dt_latent=cfg.dt_latent, len_cath_codes=len_cath_codes
    )
    ...
    # Select only important config parameters for results
    important_params = [
        'run_name_', 'ckpt_path', 'ckpt_name', 'ncpus_', 'seed',
        'nres_lens', 'min_len', 'max_len', 'step_len', 'nsamples_per_len', 'max_nsamples',
        'dt', 'self_cond', 'fold_cond', 'cath_code_level', 'len_cath_code_path',
        'guidance_weight', 'autoguidance_ratio', 'autoguidance_ckpt_path',
        'designability_seqs_per_struct', 'compute_designability', 'compute_fid',
        'dt_latent', 'ca_only', 'num_seq_per_target',
        'model_name'
    ]
    
    # Create a filtered config dict with only important parameters
    ...
    filtered_dict = {k: v for k, v in flat_dict.items() if k in important_params}
    ...
    for pred in predictions:
        coors_atom37 = pred["pred_coords"]  # [b, n, 37, 3], prediction_step returns atom37
