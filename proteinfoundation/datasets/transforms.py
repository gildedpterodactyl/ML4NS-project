...
class RandomCropTransform(T.BaseTransform):
    """Randomly crops a protein sequence to a specified length.

    This transform randomly selects a starting position and crops the sequence
    to the specified length, ensuring the crop is within the sequence boundaries.
    """
    ...
    def __init__(self, crop_size: int):
        ...
        Args:
            crop_size: Length of the crop
        """
        self.crop_size = crop_size
        
    def __call__(self, graph: Data) -> Data:
        """Randomly crops the sequence.

        Args:
            graph: PyG Data object containing protein structure

        Returns:
            Modified graph with cropped sequence
        """
        
        # Get the sequence length
        seq_length = len(graph.coord_mask)
        device = graph.coord_mask.device
        num_res_crop_size = min(int(seq_length), self.crop_size)
        ...
        for k, v in graph.items():
            crop_start = num_res_crop_start
            crop_size = num_res_crop_size
            if isinstance(v, torch.Tensor):
                if v.dim() >= 1:
                    graph[k] = v[crop_start:crop_start + crop_size]
