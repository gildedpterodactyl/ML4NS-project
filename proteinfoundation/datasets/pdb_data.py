...
class PDBDataSelector:
    def __init__(
        ...
        self.pdb_list_fpath = pdb_list_fpath
    ...
    def create_dataset(self) -> pd.DataFrame:
        ...
        if self.pdb_list_fpath:
            with open(self.pdb_list_fpath, 'r') as f:
                pdb_list = f.readlines()
            pdb_list = [file.strip() for file in pdb_list]
            pdb_manager.df = pdb_manager.df[pdb_manager.df['pdb'].isin(pdb_list)]
...
class PDBDataset(Dataset):
    def __init__(
        ...
        if self.in_memory:
            ...
            self.data = [torch.load(self.processed_dir / f"{f}.pt") for f in tqdm(file_names)]
...
class PDBLightningDataModule(BaseLightningDataModule):
    ...
    def prepare_data(self):
        if self.dataselector:
            ...
            if not self.overwrite and (self.data_dir / df_data_name).exists():
                ...
                if self.dataselector.pdb_list_fpath is None:
                    logger.info(
                        f"Dataset created with {len(df_data)} entries. Now downloading structure data..."
                    )
                    pdb_codes = sorted(list(set(df_data["pdb"].tolist())))
                    # print(len(pdb_codes))
                    # with open("pdb_files.txt", "w") as f:
                    #     for pdb in pdb_codes:
                    #         f.write(f"{pdb},")
                    self._download_structure_data(pdb_codes)
    ...
    def _process_structure_data(self, pdb_codes, chains):
        ...
        if chains is not None:
            index_pdb_tuples = [
                (i, pdb, chains[i])
                for i, pdb in tqdm(enumerate(pdb_codes), desc="Preparing args", total=len(pdb_codes))
    ...
    def _get_dataset(self, split: Literal["train", "val", "test"]) -> PDBDataset:
        ...
        if split == "test":
        #     pdb_codes = pdb_codes[:3] + ['T1109-D1', 'T1180-D1', 'T1133-D1']
            # pdb_codes = ['H1172v1_A']
            # random 50
            import random
            random.shuffle(pdb_codes)
            pdb_codes = pdb_codes[:50]
        # print(pdb_codes)
